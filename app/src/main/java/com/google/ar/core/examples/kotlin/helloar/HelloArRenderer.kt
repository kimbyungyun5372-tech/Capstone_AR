...
class HelloArRenderer(val activity: HelloArActivity) :
  ...
  fun createAnchorsAtInterval(
    ...
    if (earth == null) {
      Log.w(TAG, "earth API not available on this device.")
      return
    }
    val camAlt = earth.cameraGeospatialPose.altitude - 1.5
    val quaternion = floatArrayOf(0f, 0f, 0f, 1f)

    // 두 점 사이 거리 계산
    val results = FloatArray(1)
    Location.distanceBetween(lat1, lon1, lat2, lon2, results)
    val totalDistance = results[0].toDouble()

    if (totalDistance < intervalMeters) {
      // 너무 가까우면 시작점/끝점만 Anchor 생성
      return
    }

    val steps = (totalDistance / intervalMeters).toInt()

    for (i in 0..steps) {
      val t = i.toDouble() / steps
      val lat = lat1 * (1 - t) + lat2 * t
      val lon = lon1 * (1 - t) + lon2 * t
      val alt = camAlt
      ...
      Log.d(TAG, "Anchor added to lat = $lat, lon = $lon")
    }

    return
  }

  private fun tryCreateGeospatialAnchorOnce() {
//    Log.d(TAG, "making anchor")
    val earth = session?.earth
    if (earth == null) {
      Log.w(TAG, "earth API not available on this device.")
      return
    ...
    if (earth.trackingState != TrackingState.TRACKING) {
      Log.d(TAG, "earth is not tracking")
      return
    }

    // 이미 anchor를 만들었으면 더 만들지 않음
    if (geospatialAnchor != null) {
      Log.d(TAG, "anchor exist")
      return
    }

    val inputStream: InputStream = context.assets.open("route_coords.json")
    val jsonString = inputStream.bufferedReader().use {it.readText()}
    val jsonObject = JSONObject(jsonString)
    val coordsArray = jsonObject.getJSONArray("coords")

    // 1. 원하는 위도/경도/고도 입력
    val latitude = 37.186311
    val longitude = 127.041305
    val altitude = earth.cameraGeospatialPose.altitude - 1.5 // 지면 근처
//    val posY: Double = 81.0

    // 2. 회전 정보 (방향이 중요하지 않으면 단위 쿼터니언 사용)
    val quaternion = floatArrayOf(0f, 0f, 0f, 1f)

    for (i in 0 until coordsArray.length()) {
      val coord = coordsArray.getJSONArray(i)
      val lat = coord.getDouble(0)
      val lon = coord.getDouble(1)

      if (i + 1 < coordsArray.length()) {
        val coord2 = coordsArray.getJSONArray(i + 1)
        val lat2 = coord2.getDouble(0)
        val lon2 = coord2.getDouble(1)

        createAnchorsAtInterval(lat, lon, lat2, lon2, 5.0)
      }
    }
    ...
    Log.d("AR", "카메라 고도: ${earth.cameraGeospatialPose.altitude}")
    Log.d("AR", "생성할 Anchor 고도: $altitude")

    // 4. 기존 시스템에 등록
    wrappedAnchors.add(WrappedAnchor(anchor, null))
    geospatialAnchor = wrappedAnchors.first().anchor

//    Log.d(TAG, "Geospatial Anchor created at lat: $latitude, lon: $longitude")
  }
